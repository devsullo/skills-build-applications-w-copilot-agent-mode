...
const Users = () => {
    ...
    fetch(endpoint)
      ...
      .then((data) => {
        const results = Array.isArray(data)
          ...
          ? data.results
          : [];
        console.log('[Users] Fetched data:', data, 'Resolved items:', results);
        setItems(results);
      })
      .catch((fetchError) => {
  ...
  const filteredItems = items.filter((item) => {
    const text = `${item.username || item.name || ''} ${item.email || item.email_address || ''}`.toLowerCase();
      ...
      <div className="card shadow-sm">
        <div className="card-header d-flex justify-content-between align-items-center">
          <div>
            ...
            <p className="mb-0 text-muted">Browse user records from the backend REST API.</p>
          </div>
          <div>
                ...
                <tbody>
                  {filteredItems.map((item, index) => (
                    <tr key={item.id || index}>
                      ...
                      <td>{item.email || item.email_address || 'N/A'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div className="modal-dialog modal-dialog-centered" role="document">
            <div className="modal-content">
              <div className="modal-header">
                <h5 className="modal-title">Users API Endpoint</h5>
